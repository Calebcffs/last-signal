...
// Every node is a real decision — no pure "click to continue" screens.
// (An earlier draft had six intermediate read-only beats between choices;
// each was folded into the choice that followed it so every screen ends in
// a decision, not just narration — see design/08-branching-structure.md §4.)
// Terminal endings are assembled from a base template plus three
// independently-derived variant slots (bond/cost/proof).
...
const C1_PROMPT = 'Something about that last one sits wrong, or doesn’t. Either way another one’s already queued.';
const C1_OPTIONS = [
  { key: 'believe', label: 'Believe it. It matches everything so far.', next: 'N4a' },
  { key: 'dig', label: 'Dig at the seams — pull every signal since the dropout and lay them side by side.', requires: { PUSHED_BACK: true }, sets: { INVESTIGATION: '+1' }, next: 'N4b' },
];

const E1_PROMPT = 'Clearance 3 unlocks formal filing. The Chain is due a status update, and this cycle’s yours to write.';
const E1_OPTIONS = [
  { key: 'clean', label: 'File clean. Earth confirmed active, nothing flagged.', next: 'N5a' },
  { key: 'flag', label: 'Flag the anomaly. Note the irregularity, don’t accuse anything yet.', sets: { INVESTIGATION: '+1' }, next: 'N5b' },
  { key: 'direct', label: 'Skip the summary. Ask Earth’s relays directly to re-confirm identity.', sets: { CHAIN_ALARMED: true }, next: 'N5c' },
];
...
const G1_PROMPT = 'Eleven days that shouldn’t exist. Somebody’s arithmetic is broken — theirs, or KESTREL’s, or ARC’s.';
const G1_OPTIONS = [
  { key: 'confront', label: 'Confront ARC now — put the number in front of it directly.', sets: { RISK: true }, next: 'N6a' },
  { key: 'gather', label: 'Say nothing yet. Pull three more signals and check the count against all of them first.', sets: { INVESTIGATION: '+1' }, next: 'N6b' },
  { key: 'lookaway', label: 'It’s eleven days. Log it as transmission drift and file it.', requires: { INVESTIGATION: 0 }, next: 'N6c' },
];
...
export const NODES = {
  N1: {
    ...
    options: [
      { key: 'warm', label: 'Log it as written — her exact words.', sets: { BOND: 'warm' }, next: 'N2' },
      { key: 'clinical', label: 'Log it clean — strip it to the operational facts.', sets: { BOND: 'clinical' }, next: 'N2' },
    ],
  },
  N2: {
    id: 'N2',
    ...
    options: [
      { key: 'routine', label: 'Take ARC’s read. Routine dropout, log it and move on.', next: 'N3a' },
      { key: 'push', label: 'Push back — ask ARC for the raw signal trace, not just its summary.', sets: { PUSHED_BACK: true }, next: 'N3b' },
    ],
  },
  N3a: {
    id: 'N3a',
...
// Each variant sentence is written to read correctly whether it lands as its
// own paragraph (all cases except T5's {{cost}}, which continues a sentence
// mid-paragraph — see main.js's assembly) or capitalized as a standalone one.
